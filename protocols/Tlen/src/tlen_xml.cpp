...
void TlenXmlInitState(XmlState *xmlState)
{
	...
	xmlState->root.name = nullptr;
	xmlState->root.depth = 0;
	xmlState->root.numAttr = 0;
	xmlState->root.maxNumAttr = 0;
	xmlState->root.attr = nullptr;
	xmlState->root.numChild = 0;
	xmlState->root.maxNumChild = 0;
	xmlState->root.child = nullptr;
	xmlState->root.text = nullptr;
	xmlState->root.state = NODE_OPEN;
	xmlState->callback1_open = nullptr;
	xmlState->callback1_close = nullptr;
	xmlState->callback2_open = nullptr;
	xmlState->callback2_close = nullptr;
	xmlState->userdata1_open = nullptr;
	xmlState->userdata1_close = nullptr;
	xmlState->userdata2_open = nullptr;
	xmlState->userdata2_close = nullptr;
}
...
//						TlenLog("ATTR_MAX_LEN too small, ignore current tag");
			...
			if (q < eob) {	// found starting bracket of the next element
				...
				strncpy(str, p, q-p);
				str[q-p] = '\0';
				TlenXmlProcessElem(xmlState, ELEM_TEXT, str, nullptr);
				mir_free(str);
				num += (q-p);
				p = q;
			}
...
static void TlenXmlParseAttr(XmlNode *node, char *text)
{
	...
	if (node == nullptr || text == nullptr || mir_strlen(text) <= 0)
		return;
...
char *TlenXmlGetAttrValue(XmlNode *node, char *key)
{
	...
	if (node == nullptr || node->numAttr <= 0 || key == nullptr || mir_strlen(key) <= 0)
		return nullptr;
	for (i=0; i<node->numAttr; i++) {
...
XmlNode *TlenXmlGetNthChild(XmlNode *node, char *tag, int nth)
{
	...
	if (node == nullptr || node->numChild <= 0 || tag == nullptr || mir_strlen(tag) <= 0 || nth < 1)
		return nullptr;
	num = 1;
	for (i=0; i<node->numChild; i++) {
...
XmlNode *TlenXmlGetChildWithGivenAttrValue(XmlNode *node, char *tag, char *attrKey, char *attrValue)
{
	...
	if (node == nullptr || node->numChild <= 0 || tag == nullptr || mir_strlen(tag) <= 0 || attrKey == nullptr || mir_strlen(attrKey) <= 0 || attrValue == nullptr || mir_strlen(attrValue) <= 0)
		return nullptr;
	for (i=0; i<node->numChild; i++) {
		if (node->child[i]->name && !mir_strcmp(tag, node->child[i]->name)) {
			if ((str=TlenXmlGetAttrValue(node->child[i], attrKey)) != nullptr)
...
static void TlenXmlRemoveChild(XmlNode *node, XmlNode *child)
{
	...
	if (node == nullptr || child == nullptr || node->numChild <= 0) return;
	for (i=0; i<node->numChild; i++) {
