...
for /f %%a in ('dir plugins\*.dll /B /L') do (
	...
	if /I "%%a"=="ClientChangeNotify.dll" (%ZipIt% "%Arch%\Plugins\%%~na.zip" "Libs\Pcre16.mir")
	...
	if /I "%%a"=="CurrencyRates.dll" (%ZipIt% "%Arch%\Plugins\%%~na.zip" "Icons\Proto_CurrencyRates.dll" "Plugins\CurrencyRates\*.xml" "Plugins\CurrencyRates\QuotesChart.exe")
	...
	if /I "%%a"=="SmileyAdd.dll" (%ZipIt% "%Arch%\Plugins\%%~na.zip" "Libs\Pcre16.mir")
	if /I "%%a"=="Spamotron.dll" (%ZipIt% "%Arch%\Plugins\%%~na.zip" "Libs\Pcre16.mir" "Libs\sqlite3.mir")
	...
	if /I "%%a"=="Variables.dll" (%ZipIt% "%Arch%\Plugins\%%~na.zip" "Libs\Pcre16.mir")
