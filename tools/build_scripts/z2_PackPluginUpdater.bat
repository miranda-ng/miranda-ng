...
for /f %%a in ('dir plugins\*.dll /B /L') do (
	...
	if /I "%%a"=="ClientChangeNotify.dll" (%ZipIt% "%Arch%\Plugins\%%~na.zip")
	...
	if /I "%%a"=="Dbx_sqlite.dll"  (%ZipIt% "%Arch%\Plugins\%%~na.zip")
	...
	if /I "%%a"=="Import.dll" (%ZipIt% "%Arch%\Plugins\%%~na.zip")
	...
	if /I "%%a"=="SmileyAdd.dll" (%ZipIt% "%Arch%\Plugins\%%~na.zip")
	if /I "%%a"=="Spamotron.dll" (%ZipIt% "%Arch%\Plugins\%%~na.zip")
	...
	if /I "%%a"=="Variables.dll" (%ZipIt% "%Arch%\Plugins\%%~na.zip")
