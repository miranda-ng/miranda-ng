...
for /f %%a in ('dir plugins\*.dll /B /L') do (
	...
	if /I "%%a"=="GmailNotifier.dll" (%ZipIt% "%Arch%\Plugins\%%~na.zip" "Icons\Proto_GmailMNotifier.dll")
	...
	if /I "%%a"=="LibreView.dll" (%ZipIt% "%Arch%\Plugins\%%~na.zip" "Icons\Proto_LibreView.dll")
	...
	if /I "%%a"=="WhatsApp.dll" (%ZipIt% "%Arch%\Plugins\%%~na.zip" "Icons\Proto_WhatsApp.dll" "Libs\libsignal.mir" "Libs\libqrencode.mir")
